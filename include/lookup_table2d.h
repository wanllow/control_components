...
class LookupTable2D : public LookupTable {
 public:
  ...
  LookupTable2D(const std::size_t &rows, const std::size_t &cols)
      : row_axis_{Eigen::RowVectorXd::LinSpaced(rows, 1, rows)},
        col_axis_{Eigen::RowVectorXd::LinSpaced(cols, 1, cols)},
        map_matrix_{Eigen::MatrixXd::Zero(rows, cols)} {}
  LookupTable2D(const Eigen::RowVectorXd row_axis,
                const Eigen::RowVectorXd col_axis,
                const Eigen::MatrixXd &map_matrix) {
    table_assigned_ = AssignTableData(row_axis, col_axis, map_matrix);
  }
  LookupTable2D(const std::vector<double> &row_axis,
                const std::vector<double> &col_axis,
                const std::vector<double> &map_matrix) {
    table_assigned_ = AssignTableData(row_axis, col_axis, map_matrix);
  }
  ~LookupTable2D() = default;
 ...
 private:
  ...
  double Interpolation(const MatrixIndex &prelookup_index,
                       const double &row_value, const double &col_value);

  // Extrapolation if input is out of bounds, only support clip method
  double Extrapolation(const MatrixIndex &prelookup_index,
                       const double &row_value, const double &col_value);
};
}  // namespace control
}  // namespace wanllow