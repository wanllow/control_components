...
class LookupTable {
 public:
  ...
  class MatrixIndex {
   public:
    ...
    MatrixIndex(const std::size_t &rows, const std::size_t &cols)
        : rows_{rows}, cols_{cols} {}
    // explicit MatrixIndex(Eigen::Index rows, Eigen::Index cols) :
  ...
  virtual void SetExtrapMethod(const ExtrapMethod &method) {
    extrap_method_ = method;
  }
  void SetEpsilon(const double &epsilon) {
    epsilon_ = epsilon > 0 ? epsilon : epsilon_;
  }

  // virtual void SetTable() = 0;   // SetTable without any input parameter,
  // it's complete virtual function
  virtual bool
  ClearTable() = 0;  // ClearTable may be different for 1dTable and 2dTable

  // Three main search index functions
  std::size_t SearchIndex(const double &value, const Eigen::RowVectorXd &table,
                          const SearchMethod &method,
                          const std::size_t &last_index = 0);
 ...
 protected:
  ...
  const std::size_t max_table_size_ =
      1000000U;  // do not exceed 1M, uint32_t can support up to 4294967295U.

  // Functions commonly used
  bool isStrictlyIncreasing(const Eigen::RowVectorXd &input_vector);

  // Convert Eigen::Index (long int) type to std::size_t (unsigned long int),
  // avoid negative integers
  inline std::size_t ConvertSizeDataType(const Eigen::Index &eigen_index) {
    return static_cast<std::size_t>(std::max(eigen_index, Eigen::Index(0)));
  }

  // Interpolation basic form
  double Interpolate(const double &xvalue, const double &x1, const double &x2,
                     const double &y1, const double y2) const;
  double Interpolate(const double &rvalue, const double &cvalue,
                     const double &r1, const double &r2, const double &c1,
                     const double &c2, const double &m11, const double &m12,
