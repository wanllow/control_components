...
void TestTable2D() {
  ...
  Eigen::RowVectorXd row_axis =
      Eigen::Map<const Eigen::RowVectorXd>(row_vec.data(), 4);
  Eigen::RowVectorXd col_axis =
      Eigen::Map<const Eigen::RowVectorXd>(col_vec.data(), 3);
  Eigen::MatrixXd map_matrix =
      Eigen::Map<const Eigen::MatrixXd>(map_vec.data(), 3, 4).transpose();
  for (int i = 0; i != 4; ++i) {
    for (int j = 0; j != 3; ++j) {
