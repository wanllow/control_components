...
namespace wanllow {
namespace control {
std::size_t LookupTable::SearchIndex(const double &value,
                                     const Eigen::RowVectorXd &table,
  ...
  switch (method) {
    ...
    case SearchMethod::bin: {
      ...
      while (left < right) {
        std::size_t mid =
            left + (right - left) /
                       2;  // Avoid overflow with safer midpoint calculation
    ...
    case SearchMethod::near: {
      std::size_t index =
          last_index;  // Start searching from the last known index
      ...
      // Handle case where index is at the beginning of the table
        ...
        while (index < table.size() && value > table(index)) {
          ++index;  // Move forward until the value fits within the table's
                    // range
        }
      }
      // Forward search: if the new value is greater than or equal to the
      // current position
      else if (value >= table(index)) {
        while (index < table.size() && value > table(index)) {
          ++index;  // Continue forward search until the value fits within the
                    // range
        }
      }
      // Backward search: if the new value is smaller than the previous position
        ...
        while (index > 0 && value < table(index - 1)) {
          --index;  // Continue backward search until the value fits within the
                    // range
        }
      }
...
double LookupTable::Interpolate(const double &xvalue, const double &x1,
...
double LookupTable::Interpolate(const double &rvalue, const double &cvalue,
  ...
  return (1 - rweight) * ((1 - cweight) * m11 + cweight * m12) +
