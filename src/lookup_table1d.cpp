...
LookupTable::AssignmentState LookupTable1D::AssignTableData(
    const Eigen::RowVectorXd &x_axis, const Eigen::RowVectorXd &y_table) {
  if (CheckTableState(x_axis, y_table) == TableState::valid) {
    x_axis_ = x_axis;
    y_table_ = y_table;
    bool refresh =
        RefreshTableState();  // redundant check, and refresh state in table
    return table_valid_ ? AssignmentState::success : AssignmentState::fail;
  } else {
    // if the input value is invalid, check the current table value to set the
    // table_state flag
...
LookupTable::AssignmentState LookupTable1D::AssignTableData(
  ...
  // this is vector edition, first convert then call the base AssignTableData
  // function.
  Eigen::RowVectorXd x_axis =
      Eigen::Map<const Eigen::RowVectorXd>(x_vec.data(), x_vec.size());
  Eigen::RowVectorXd y_table =
      Eigen::Map<const Eigen::RowVectorXd>(y_vec.data(), y_vec.size());
  return AssignTableData(x_axis, y_table);
}
// AssignTableData is related with three functions: CheckTableState,
// RefreshTableState, ClearTable.
...
LookupTable::TableState LookupTable1D::CheckTableState(
    const Eigen::RowVectorXd &input_vector1,
    const Eigen::RowVectorXd &input_vector2) {
  if (input_vector1.size() == 0 || input_vector2.size() == 0) {
    return TableState::empty;
  } else if (input_vector1.size() > max_table_size_ ||
             input_vector1.size() < 2) {
    return TableState::size_invalid;  // x table size must be within the range
                                      // of (2 max_axis_size)
...
std::size_t LookupTable1D::PreLookup(const double &xvalue) {
  size_t prelook_index =
      SearchIndex(xvalue, x_axis_, search_method_, prelook_index_);
...
double LookupTable1D::Interpolation(const std::size_t &index,
                                    const double &xvalue) {
...
double LookupTable1D::InterpolationLinear(const std::size_t &index,
                                          const double &xvalue) {
  // Interpolate 1D
  return Interpolate(xvalue, x_axis_(index - 1), x_axis_(index),
                     y_table_(index - 1), y_table_(index));
}
double LookupTable1D::InterpolationNearest(const std::size_t &index,
                                           const double &xvalue) {
  return ((xvalue - x_axis_(index - 1)) <= (x_axis_(index) - xvalue))
             ? y_table_(index - 1)
             : y_table_(index);
}
double LookupTable1D::InterpolationNext(const std::size_t &index,
                                        const double &xvalue) {
  return y_table_(index);
}
double LookupTable1D::InterpolationPrevious(const std::size_t &index,
                                            const double &xvalue) {
  return y_table_(index - 1);
}

// Extrapolation if input is out of bounds
double LookupTable1D::Extrapolation(const std::size_t &index,
                                    const double &xvalue) {
  switch (extrap_method_) {
    ...
    case ExtrapMethod::specify:
      return ExtrapolationSpecify(index, lower_extrap_value_specify_,
                                  upper_extrap_value_specify_);
    default:
      bool refresh = RefreshTableState();
      return lookup_result_;
  }
...
double LookupTable1D::ExtrapolationLinear(const std::size_t &index,
                                          const double &xvalue) {
  if (index == 0) {
    double x1 = x_axis_(0);
    double x2 = x_axis_(1);
...
double LookupTable1D::ExtrapolationSpecify(const std::size_t &index,
                                           const double &lower_extrap_value,
                                           const double &upper_extrap_value) {
  if (index == 0) {
...
double LookupTable1D::Lookup(const double &xvalue) {
  if (table_valid_) {
    ...
    xvalue_ = xvalue;  // restore the input value to member variable, for use in
                       // some cases.
  } else {
    bool refresh = RefreshTableState();
  }
