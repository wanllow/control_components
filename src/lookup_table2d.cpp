...
LookupTable::AssignmentState LookupTable2D::AssignTableData(
    const Eigen::RowVectorXd &row_axis, const Eigen::RowVectorXd &col_axis,
    const Eigen::MatrixXd &map_matrix) {
...
LookupTable::AssignmentState LookupTable2D::AssignTableData(
  ...
  if (rows * cols == map_vec.size()) {
    Eigen::RowVectorXd row_axis =
        Eigen::Map<const Eigen::RowVectorXd>(row_vec.data(), rows);
    Eigen::RowVectorXd col_axis =
        Eigen::Map<const Eigen::RowVectorXd>(col_vec.data(), cols);
    Eigen::MatrixXd map_matrix =
        Eigen::Map<const Eigen::MatrixXd>(map_vec.data(), cols, rows)
            .transpose();
    return AssignTableData(row_axis, col_axis, map_matrix);
  } else {
    bool valid = RefreshTableState();  // refresh the table
    return valid ? AssignmentState::remain
                 : AssignmentState::fail;  // return assignment state
  }
}
...
LookupTable::TableState LookupTable2D::CheckTableState(
    const Eigen::RowVectorXd &input_vector1,
    const Eigen::RowVectorXd &input_vector2,
    const Eigen::MatrixXd &input_matrix) {
  if (input_vector1.data() == nullptr && input_vector2.data() == nullptr &&
      input_matrix.data() == nullptr) {
    return LookupTable::TableState::empty;
  } else if (input_vector1.size() < 2 ||
             input_vector1.size() > max_table_size_ ||
             input_vector2.size() < 2 ||
             input_vector2.size() > max_table_size_ ||
             input_matrix.rows() < 2 || input_matrix.rows() > max_table_size_ ||
             input_matrix.cols() < 2 || input_matrix.cols() > max_table_size_) {
...
LookupTable::MatrixIndex LookupTable2D::PreLookup(const double &rvalue,
                                                  const double &cvalue) {
  std::size_t row =
      SearchIndex(rvalue, row_axis_, search_method_, prelook_index_.rows());
  std::size_t col =
      SearchIndex(cvalue, col_axis_, search_method_, prelook_index_.cols());
...
double LookupTable2D::Interpolation(const MatrixIndex &prelookup_index,
                                    const double &rvalue,
                                    const double &cvalue) {
...
double LookupTable2D::Extrapolation(const MatrixIndex &prelookup_index,
                                    const double &rvalue,
                                    const double &cvalue) {
  // Setting calculation range
  const std::size_t &rindex = prelookup_index.rows();
  ...
  if (rindex < 1) {
    if (cindex < 1) {
      ...
      // Interpolate 1D
      result = Interpolate(cvalue, col_axis_(cindex - 1), col_axis_(cindex),
                           map_matrix_(0, cindex - 1), map_matrix_(0, cindex));
    }
    ...
    if (cindex < 1) {
      ...
      // Interpolate 1D
      result = Interpolate(cvalue, col_axis_(cindex - 1), col_axis_(cindex),
                           map_matrix_(rsize - 1, cindex - 1),
                           map_matrix_(rsize - 1, cindex));
    }
  } else {
    if (cindex < 1) {
      // Interpolate 1D
      result = Interpolate(rvalue, row_axis_(rindex - 1), row_axis_(rindex - 1),
                           map_matrix_(rindex - 1, 0), map_matrix_(rindex, 0));
    } else if (cindex >= csize) {
      // Interpolate 1D
      result = Interpolate(rvalue, row_axis_(rindex - 1), row_axis_(rindex),
                           map_matrix_(rindex - 1, csize - 1),
                           map_matrix_(rindex, csize - 1));
    }
  }
  return result;
}
